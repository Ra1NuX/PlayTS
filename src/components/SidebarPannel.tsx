...
export const DependenciesPanel = () => {
  ...
  const [installedDependecies, setInstalledDependencies] = useState<{[x: string]: string} | null>(null);
  ...
  useEffect(() => {
    setInstalledDependencies(JSON.parse(localStorage.getItem('dependencies') || '{}'));
  },[]);
  ...
  return (
    <>
      ...
      <input
        ...
        name="dependencies"
        id="dependencies"
        className="dark:bg-main-light dark:text-white shadow-md p-2 flex m-2 mx-auto w-full rounded-lg"
        onChange={(e) => {
          searchFunction(e);
        }}
      />
      <section className="flex flex-1 flex-col h-auto overflow-y-auto">
        {search ?
          info?.objects.map((element) => {
            return (
              <div key={element.package.name} className="p-2 flex m-2 mx-auto w-full rounded-lg justify-between items-center">
                <h2 className="dark:text-gray-300 font-semibold  flex flex-col text-left w-1/2 break-words">
                  {element.package.name}
                  <span className="dark:text-gray-500 font-semibold text-start text-xs">
                    {element.package.version}
                    </span>
                </h2>
                  <DownloadPackageButton pckg={element.package.name} version={element.package.version} />
              </div>
            );
          }) : 
          <>
            {installedDependecies && Object.keys(installedDependecies).length > 0 ? 
              <h2 className="dark:text-gray-300 font-semibold flex flex-col text-left pt-4">
                <span className="flex items-center gap-2 px-2">
                <BiSolidStar className="" color="#d8e548" /> {t('INSTALLED_DEPENDENCIES')}:
                </span>
